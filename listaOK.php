...
<?php  
 ...
 // $query =	"select idtiporef, docref,
	...
	$query =	"select idtiporef, docref,
						...
						from tbpedido, tbpersona where tbpedido.idcli = tbpersona.idpersona
						and fecemi is not null and fecemi >= date('2018-01-23') and fecemi <= date('2018-01-23')
						and rtrim(substring(docref from 1 for position(' ' in docref)),' ') = '010'
						and ltrim(substring(docref from position('-' in docref)+1 for 2)) = 'F'
						and idtiporef in (
						    select idref from tbctas_cobrar 
						    where fecemi >= date('2018-01-23') and fecemi <= date('2018-01-23') 
...
*/
	...
	$query = "select idsalida,
						...
						and ltrim(substring(tbsalida.nrodoc from position('-' in tbsalida.nrodoc)+1 for 2)) = 'F'
						and idsalida in (
								select idref from tbctas_cobrar 
								where fecemi >= date('2018-07-27') and fecemi <= date('2018-08-01') 
