<?php 
 ...
 // 	                 idmoneda
	...
	$query =	"select idtiporef, docref,
						...
						from tbpedido, tbpersona where tbpedido.idcli = tbpersona.idpersona
						and fecemi is not null and fecemi >= date('2018-03-14') and fecemi <= date('2018-03-17')
						...
						and idtiporef in (
						    ...
						    where fecemi >= date('2018-03-14') and fecemi <= date('2018-03-17') 
	...
	$docref = $fila[1];
 	...
 	if ($fila[8]==null) {
	 	if ($fila['otrodoc']==null) {
	 		...
	 		$cli_nrodoc=$fila['otrodoc'];
	 	}
 	}

 	if($idtipodocid == '-'){
 		$cli_nrodoc = $idtiporef;
 	}
 ...
 // 	echo $moneda."<br>";
	...
	$cad = $cad.'cliente_tipo_de_documento|'.$idtipodocid.'|'.$salto;
	...
	// $cad = $cad.'fecha_de_emision|'.'06-08-2017'.'|'.$salto;
...
/*
	$query = "select iddetpedpres,idpedido,idpresprod,cantidad,porcentaje1,porcentaje2,precsal,precosto,boni,des1,des2,proceso_guia,coalesce(nombre_producto, '') 
 from tbdetpedpres, tbproducto where tbdetpedpres.idpresprod = tbproducto.idprod and idpedido =".$idpedido;
*/
...
/*
	$conexion = pg_connect($cadenaConexion) or die("Error en la Conexión: ".pg_last_error());
	$query = "select iddetpedpres,idpedido,idpresprod,cantidad,porcentaje1,porcentaje2,precsal,precosto,boni,des1,des2,proceso_guia,coalesce(nombre_producto, '') || ' ' ||
coalesce((select tbs.nroserie from tbseries tbs inner join tbseriesped tbsped on (tbsped.idserie=tbs.idserie)
						inner join tbdetpedpres tbdet  on(tbdet.iddetpedpres=tbsped.iddetpedpres) inner join tbpedido tbp on(tbp.idpedido=tbdet.idpedido)
							where tbp.idpedido=tbdetpedpres.idpedido
						 ), '')
 from tbdetpedpres, tbproducto where tbdetpedpres.idpresprod = tbproducto.idprod and idpedido =".$idpedido;
*/
